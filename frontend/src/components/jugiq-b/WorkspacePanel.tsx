...
import { CircleCheck, CornerDownRight, Scale, TriangleAlert } from "lucide-react";
    ...
    <div className="mt-0.5">
      ...
      {conflict && (
        <p
          ...
          data-testid="workspace-booking-conflict"
        >
...
export function WorkspacePanel({
  ...
  return (
    <div className={cn("flex flex-col bg-parchment", className)} data-testid="workspace-panel">
      {/* Layer 3 — what needs attention */}
      <div className="border-b border-hairline px-5 py-4" data-testid="workspace-attention">
        ...
        {nextItem && (
          <button
            ...
            data-testid="workspace-next-attention"
          >
      ...
      <div className="flex-1 overflow-y-auto">
        {groupStance && (
          <section className="border-b border-hairline px-5 py-4" data-testid="workspace-group-stance">
            <h3 className="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
              Where the group stands
            </h3>
            <p className="mt-2 text-[0.7rem] font-medium uppercase tracking-wide text-pine">
              Broadly agreed
            </p>
            <ul className="mt-1 space-y-1">
              {groupStance.agreed.map((a) => (
        ...
        {/* Layer 2 — the adopted Current Plan, as a route spine */}
        <section className="px-5 py-4">
          ...
          >
            ...
            <ol className="space-y-5">
              {dated.map((s, i) => (
                <li key={s.id} className="relative" data-testid={`workspace-stop-${s.id}`}>
                  <span className="absolute -left-[1.03rem] top-1 size-2.5 rounded-full bg-foreground ring-4 ring-parchment" />
                  <div className="flex items-baseline justify-between gap-2">
                    <h4 className="font-heading text-[1.05rem] leading-none">
                      {s.city}
                    </h4>
                          ...
                          {s.stayStatus === "booked" && s.stayBooking ? (
                            ...
                            <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5">
                              <button
                                ...
                                className="text-[0.72rem] font-medium text-clay transition-colors hover:text-clay/80"
                                data-testid={`ws-mark-booked-stay-${s.id}`}
                              >
                                Mark as booked
                              </button>
                              {s.stayNudge && s.stayBlockedBy && openIds.has(s.stayBlockedBy) && (
                                <button
                                  ...
                                  className="text-left text-[0.72rem] leading-snug text-muted-foreground transition-colors hover:text-foreground"
                                  data-testid={`ws-nudge-${s.stayBlockedBy}`}
                                >
                                  {s.stayNudge}
                                </button>
                              )}
                            ...
                            {h.status === "booked" && h.booking ? (
                              ...
                              <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5">
                                <button
                                  ...
                                  className="text-[0.72rem] font-medium text-clay transition-colors hover:text-clay/80"
                                  data-testid={`ws-mark-booked-activity-${h.label.slice(0, 10).toLowerCase().replace(/\s+/g, "-")}`}
                                >
                                  Mark as booked
                                </button>
                                {h.nudge && h.blockedBy && openIds.has(h.blockedBy) && (
                                  <button
                                    ...
                                    className="text-left text-[0.72rem] leading-snug text-muted-foreground transition-colors hover:text-foreground"
                                    data-testid={`ws-nudge-${h.blockedBy}`}
                                  >
                                    {h.nudge}
                                  </button>
                                )}
          ...
          <ul className="mt-2 divide-y divide-hairline">
            {plan.decisions.map((d) => (
              <li
                ...
                className={cn(
                  "rounded-md py-3 transition-colors",
                  focused === d.id && "animate-glow",
                )}
                data-testid={`ws-decision-${d.id}`}
              >
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
                  ...
                  {d.options.slice(0, 2).map((o) => (
                    <button
                      ...
                      data-testid={`ws-decide-${d.id}-${o.slice(0, 10).toLowerCase().replace(/\s+/g, "-")}`}
                    >
                      {o}
                    </button>
                  ))}
