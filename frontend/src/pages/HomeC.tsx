...
import { PlanPanelC, type AdoptedStance } from "@/components/jugiq-c/PlanPanelC";
import {
  AGREED_SLOW_DAY,
  CHANGE_MESSAGES,
...
function scriptedReply(text: string, hasPlan: boolean): { text: string; artifact: Artifact } {
  ...
  if (/ferry|bridge|cross/.test(t))
    return {
      text: "Ferry is quicker from Tsim Sha Tsui; the bridge coach is cheaper and steadier in wind. I've left it in your plan as an open decision — it's a genuine trade-off.",
      artifact: null,
    };
  if (/price|cost|book|cheap|compare|ticket/.test(t))
    return {
      text: "Open Compare on the Peak Tram decision and I'll lay the sources side by side — ranked on real traveller value, not who pays us.",
      artifact: null,
    };
  return {
    text: "Noted — I've kept your plan in sync. Nothing moves until a change is big enough to be worth showing you side by side first.",
    artifact: null,
  };
}
...
export default function HomeC() {
  ...
  // Group semantics: no agreement claims before Revise; only a neutral signal.
  const newMessagesNote = isGroup && !groupApplied ? "5 new messages since the last plan update" : undefined;
  const adoptedStance: AdoptedStance | undefined =
    isGroup && groupApplied
      ? {
          agreed: ["Hong Kong 5 nights", "Macau 3 nights", "A slow, unstructured day in Hong Kong"],
          unresolved: ["Saturday show vs early Sunday flight"],
        }
      : undefined;
  ...
  function send() {
    ...
    if (isGroup && !/@jugiq/i.test(text)) return; // human chatter never invokes JugIQ
  ...
  function usePlan() {
    ...
    setMessages([
      ...
      { id: nextId(), kind: "user", time: "now", text: "Let's go with this." },
      {
        ...
        kind: "jugiq",
        time: "now",
        text: "Done — it's your Current Plan now, alongside the chat. Everything's planned, nothing's booked. Mark anything as booked once you've actually locked it in.",
      },
    ]);
  ...
  function renderArtifact(msg: ChatMsg) {
    if (msg.artifact === "sketch")
      return <SketchTimeline onUsePlan={usePlan} onAdjust={() => setMobilePlanOpen(true)} />;
  ...
  const planPanel = (
    <PlanPanelC
      plan={plan}
      newMessagesNote={newMessagesNote}
  ...
  return (
    <div className="flex h-dvh flex-col bg-background">
        ...
        <div className="mx-auto flex max-w-[96rem] items-center gap-3 px-4 py-3 sm:px-6">
          ...
          {planActive && (
            <div className="ml-auto flex items-center gap-2">
              ...
              {/* desktop: collapse / reopen the persistent plan */}
              {planCollapsed ? (
                <Button
                  variant="outline"
                  size="sm"
                  ...
                  data-testid="show-plan-button"
                >
                  <ListChecks className="size-4" />
                  Plan · {attentionCount} need attention
                </Button>
              ) : (
                <Button
                  variant="ghost"
                  size="sm"
                  className="hidden lg:inline-flex"
                  onClick={() => setPlanCollapsed(true)}
                  data-testid="hide-plan-button"
                >
      ...
      <div className="mx-auto flex w-full max-w-[96rem] min-h-0 flex-1">
              ...
              {showHero && (
                <div className="pb-2 pt-6 sm:pt-12" data-testid="new-trip-hero">
                  ...
                  <p className="mt-3 max-w-lg text-[0.98rem] leading-relaxed text-muted-foreground">
                    ...
                    about. We'll shape the plan together as we talk.
                  </p>
                </div>
              )}
        ...
        {planActive && !planCollapsed && (
          <aside
            className="hidden shrink-0 border-l border-hairline lg:flex lg:w-[35%] xl:w-[34%]"
      ...
      <Sheet open={mobilePlanOpen} onOpenChange={setMobilePlanOpen}>
        <SheetContent side="bottom" className="h-[88dvh] p-0" data-testid="mobile-plan-sheet">
